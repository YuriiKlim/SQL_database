...
JOIN 
    DEPARTMENTS ON DONATIONS.DEPARTMENT_ID = DEPARTMENTS.ID
JOIN 
    DOCTORS ON DEPARTMENTS.ID = DOCTORS.DEPARTMENT_ID
WHERE 
    DONATIONS.AMOUNT > 40000;
...
FROM 
    DOCTORS
JOIN 
    DEPARTMENTS ON DOCTORS.DEPARTMENT_ID = DEPARTMENTS.ID
WHERE 
    DOCTORS.PREMIUM = 0;
...
FROM 
    EXAMINATIONS
JOIN 
    DEPARTMENTS ON EXAMINATIONS.DEPARTMENT_ID = DEPARTMENTS.ID
JOIN 
    DISEASES ON EXAMINATIONS.DISEASE_ID = DISEASES.ID
WHERE 
    EXAMINATIONS.EXAMINATION_DATE >= (CURRENT_DATE - INTERVAL '6 months');
